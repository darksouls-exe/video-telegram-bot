...
def clean_url(value):
    ...
    if host in {"fb.watch", "fb.gg"}:
        try:
            response = requests.get(value, headers={"User-Agent": UA},
    ...
    # Numeric Reel/Watch/Video links are easier for yt-dlp in mobile form.
    patterns = (
...
def download_with_ytdlp(url, height):
    ...
    # Progressive MP4 is tried first so Render does not need ffmpeg.
    formats = (
        ...
        for fmt in formats:
            try:
                with yt_dlp.YoutubeDL(ydl_options(
                    url, outtmpl=f"{prefix}.%(ext)s", format=fmt
                )) as ydl:
                    ydl.download([url])
                files = [p for p in glob.glob(prefix + ".*") if not p.endswith(".part")]
                if files:
                    return max(files, key=os.path.getsize)
            except Exception as error:
                last_error = error
                if any(word in str(error).lower()
                       for word in ("cannot parse", "unsupported url", "please report")):
                    update_ytdlp()
                    continue
        raise last_error
    except Exception:
...
def download_file(session, url, filename):
    with session.get(url, headers={"Referer": "https://snapsave.app/"},
                     stream=True, timeout=90) as response:
        response.raise_for_status()
        if "text/html" in response.headers.get("content-type", "").lower():
            raise RuntimeError("dịch vụ trung gian không trả về video")
...
def download_via_snapsave(url, height):
    ...
    result_soup = BeautifulSoup(html.unescape(result.text), "html.parser")
    links = []
    for anchor in result_soup.select("a[href]"):
        href = anchor.get("href", "")
        label = anchor.get_text(" ", strip=True).lower()
        if href.startswith("http") and (
            any(word in href.lower() for word in ("fbcdn", "facebook", "video"))
            or "download" in label
        ):
            quality = 1080 if "hd" in label else 480 if "sd" in label else 360
            links.append((abs(quality - height), href))
    if not links:
        raise RuntimeError("SnapSave không trả về link video")
    _, video_url = min(links)
    return download_file(session, video_url, f"video_{uuid4().hex}.mp4")
...
def download_video(url, height):
    try:
        ...
        try:
            ...
            if any(word in error_text for word in ("private", "login", "sign in")):
...
def buttons():
    ...
    keyboard.row(InlineKeyboardButton("360p", callback_data="res:360"),
                 InlineKeyboardButton("480p", callback_data="res:480"))
    keyboard.row(InlineKeyboardButton("720p", callback_data="res:720"),
...
def upload_cookie():
    # This is for the owner only. Telegram users never need this page.
    ...
    if request.method == "POST":
        cookie = request.form.get("cookie", "").strip().lstrip("\ufeff")
        ...
        with open(COOKIE_FILE, "w", encoding="utf-8") as output:
            output.write(cookie)
        return f"Facebook cookie saved on server. Cookie lines: {len(cookie.splitlines())}"
    return f"""<meta charset="utf-8"><h2>Facebook cookie</h2>
<p>Chỉ chủ bot dùng trang này.</p>
<form method="post"><input type="hidden" name="key" value="{html.escape(supplied)}">
<textarea name="cookie" rows="20" cols="90"></textarea><br>
<button>Lưu cookie</button></form>"""


@app.get("/cookie-status")
def cookie_status():
    """Admin-only status check; never returns cookie contents."""
    key = os.getenv("COOKIE_UPLOAD_KEY", "").strip()
    supplied = request.args.get("key", "").strip()
    if not key or supplied != key:
        return "Not found", 404
    if not os.path.isfile(COOKIE_FILE):
        return "NO_COOKIE_FILE", 404
    try:
        with open(COOKIE_FILE, encoding="utf-8") as source:
            lines = source.readlines()
        valid_header = bool(lines) and lines[0].lstrip("\ufeff").startswith(
        ...
        return {
            "file": "present",
            "format": "netscape" if valid_header else "unknown",
            "lines": len(lines),
...
def receive(message):
    try:
        ...
        if is_facebook(url):
            bot.reply_to(message, "🎬 Chọn độ phân giải:", reply_markup=buttons())
            return
        bot.reply_to(message, "🔍 Đang kiểm tra link...")
        with yt_dlp.YoutubeDL(ydl_options(url, skip_download=True)) as ydl:
            ydl.extract_info(url, download=False)
        bot.send_message(message.chat.id, "🎬 Chọn độ phân giải:", reply_markup=buttons())
    except Exception as error:
...
def choose_resolution(call):
    try:
        ...
        bot.edit_message_text(
            f"⏳ Đang tải {height}p...", call.message.chat.id, call.message.message_id
        )
        ...
        if os.path.getsize(filename) <= 50_000_000:
            with open(filename, "rb") as video_file:
                if filename.lower().endswith(".mp4"):
                    ...
                    bot.send_document(call.message.chat.id, video_file)
            os.remove(filename)
            return
        name = uuid4().hex
        large_files[name] = filename
        threading.Thread(target=delete_later, args=(name, filename), daemon=True).start()
        ...
        if code == "FACEBOOK_PRIVATE":
            ...
            message = "❌ Không tải được video. Hãy thử lại với link công khai khác."
